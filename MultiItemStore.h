...
namespace ndtech {
  ...
  struct MultiItemStore {
    ...
    void AddTupleItemToVectorFromTupleByIndex(std::tuple<IdType, ItemType...> tuple, std::tuple<std::vector<IdType>, std::vector<ItemType>...>& tupleOfItemTypeVectors) {
    ...
    template<size_t Index>
    void UpdateTupleItemToVectorFromTupleByIndex(std::tuple<IdType, ItemType...> tuple, std::tuple<std::vector<IdType>, std::vector<ItemType>...>& tupleOfItemTypeVectors, size_t idIndex) {
    ...
    template<size_t Index>
    void RemoveFromTupleOfVectorsByIndexImpl(size_t idIndex, std::tuple<std::vector<IdType>, std::vector<ItemType>...>& tupleOfItemTypeVectors) {
    ...
    std::tuple<ItemType...> GetItem(IdType itemId) {
      ...
      while (iter == keys.end()) {
        m_itemsCV.wait(lock);
        keys = std::get<0>(m_items);
        iter = std::find(keys.begin(), keys.end(), itemId);
      }
